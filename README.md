...
Получается, что мы можем переписать предыдуший пример, поместив все распределения в один файл - distributions.js в 
корне библиотеки, а не создавая отдельный каталог. После сборки код библиотеки будет эквивалентен.
```javascript
